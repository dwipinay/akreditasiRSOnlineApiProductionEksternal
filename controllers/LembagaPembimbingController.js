const lembagaPembimbing = require('../models/LembagaPembimbing')
const pagination = require('../configs/Pagination')

class LembagaPembimbingController {
    index(req, res) { 
        const lembagaPembimbingObject = new lembagaPembimbing()
        lembagaPembimbingObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = LembagaPembimbingController