const capaianAkreditasi = require('../models/CapaianAkreditasi')
const pagination = require('../configs/Pagination')

class CapaianAkreditasiController {
    index(req, res) { 
        const capaianAkreditasiObject = new capaianAkreditasi()
        capaianAkreditasiObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = CapaianAkreditasiController